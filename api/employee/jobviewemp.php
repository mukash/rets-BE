<?php
	...
	if(mysqli_num_rows($query)==true){
		...
		$id=$fetch_id['emid'];
		$sql = "SELECT jobs.*,client.cid,client.name,client.ph_number FROM  jobs INNER JOIN client ON client.cid=jobs.client_id WHERE emp_id=$id AND status='Pending'";
		...
		//$firstRow = mysqli_fetch_assoc($query2);	
